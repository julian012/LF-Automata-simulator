...
export class ProjectComponent implements OnInit, OnDestroy {
  ...
  loadProject(file: File, parseFunction: Function) {
    ...
    reader.onload = ($event: any) => {
      try {
        const rawProject = JSON.parse($event.target.result);
  ...
  saveProject(filter: Function) {
    ...
    const blob = new Blob([jsonProject], { type: 'text/plain' });
  ...
  protectAgainstUnsavedChanges(acknowledgeFunction: Function) {
    if (typeof this.project !== 'undefined' && this.project.metadata.isUnsaved) {
      const message = 'Warning: You have unsaved changes in your project. If you continue all changes will be lost.';
